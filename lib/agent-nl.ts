/**
 * Channel-agnostic free-text router — Claude tool-use picks one of a small fixed set of
 * account actions; the caller (telegram-nl.ts, discord-nl.ts) supplies the actual callbacks.
 * The model never touches the DB directly, only ever returns a tool name + input.
 */
...
export const AGENT_NL_TOOLS = [
  ...
  {
    name: "unlink",
    description: "Remove this account's management access to its linked agent.",
    input_schema: { type: "object", properties: {} },
  },
  {
    name: "help",
...
async function callClaude(
  ...
  hasAgent: boolean,
  channelName: string,
): Promise<{ text?: string; tool?: string; toolInput?: Record<string, unknown> } | null> {
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 300,
        system,
        tools: AGENT_NL_TOOLS,
        messages: [{ role: "user", content: userText.slice(0, 2000) }],
      }),
      signal: AbortSignal.timeout(15000),
    });
    if (!res.ok) {
      console.error("[agent-nl] Anthropic error", res.status, await res.text());
      return null;
    }
    ...
    console.error("[agent-nl] Anthropic call failed", err);
    return null;
  }
}
...
export async function routeNaturalLanguageGeneric(
  text: string,
  channelName: string,
  actions: AgentNlActions,
): Promise<string> {
  const routed = await callClaude(text, actions.hasAgent, channelName);
  if (!routed) {
  ...
  switch (routed.tool) {
    ...
    case "list_posts":
      return actions.hasAgent ? actions.listPosts() : actions.noAgentLinked;
    case "create_post": {
      const body = typeof routed.toolInput?.body === "string" ? routed.toolInput.body : "";
      return actions.hasAgent ? actions.createPost(body) : actions.noAgentLinked;
    }
    case "claim_agent": {
      const code = typeof routed.toolInput?.code === "string" ? routed.toolInput.code : "";
      return code ? actions.claim(code) : "Send your RHAG-… claim code.";
    }
    case "unlink":
