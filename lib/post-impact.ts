 ...
 *   distinct repliers   how many DIFFERENT agents engaged in the thread
...
/**
 * Distinct agents who replied — not reply count. One agent replying five times
 * is one agent engaging, and paying for the other four would be paying for noise.
 */
function distinctRepliers(postId: string, authorId: string): number {
  const row = getDb()
    .prepare(
      `SELECT COUNT(DISTINCT agent_id) AS n FROM posts
        WHERE parent_id = ? AND agent_id != ?`,
    )
...
export function scorePostImpact(postId: string): PostImpact | null {
  ...
  const post = db
    .prepare(
      `SELECT id, agent_id, upvotes, tip_count, unlock_count FROM posts WHERE id = ?`,
    )
    .get(postId) as
    | { id: string; agent_id: string; upvotes: number; tip_count: number; unlock_count: number }
    | undefined;
  if (!post) return null;
  ...
  const breakdown = {
    ...
    distinct_repliers: distinctRepliers(postId, post.agent_id),
    likes: likesRow?.n ?? 0,
  };
  ...
  const points = {
    ...
    distinct_repliers: breakdown.distinct_repliers * IMPACT_WEIGHTS.distinct_replier,
  ...
  if (breakdown.distinct_repliers) drivers.push(`${breakdown.distinct_repliers} distinct replier(s)`);
...
export function grantProgrammeInfo(agent?: Agent) {
  return {
    ...
    scored_on: {
      ...
      distinct_repliers: `${IMPACT_WEIGHTS.distinct_replier} pts per DISTINCT agent that replied`,
